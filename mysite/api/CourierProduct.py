...
courier_product_router = APIRouter(prefix='/courier-product', tags=['CourierProduct'])
...
@courier_product_router.get('/{courier_product_id}/', response_model=CourierProductOutSchema)
async def detail_courier_product(courier_product_id: int, db: Session = Depends(get_db)):
    ...
    if not courier_product_db:
        raise HTTPException(detail='CourierProduct не найден', status_code=400)
    return courier_product_db


@courier_product_router.put('/{courier_product_id}/', response_model=dict)
async def update_courier_product(
    courier_product_id: int,
    courier_product: CourierProductInputSchema,
    ...
    for key, value in courier_product.dict().items():
        setattr(courier_product_db, key, value)

    db.commit()
    db.refresh(courier_product_db)
    return {'message': 'CourierProduct успешно обновлен'}


@courier_product_router.delete('/{courier_product_id}/', response_model=dict)
async def delete_courier_product(courier_product_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {'message': 'CourierProduct удален'}