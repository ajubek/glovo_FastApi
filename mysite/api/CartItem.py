...
cart_item_router = APIRouter(prefix='/cart-item', tags=['CartItem'])
...
async def detail_cart_item(cart_item_id: int, db: Session = Depends(get_db)):
    ...
    if not cart_item_db:
        raise HTTPException(detail='CartItem не найден', status_code=400)
    return cart_item_db
...
async def update_cart_item(cart_item_id: int, cart_item: CartItemInputSchema,
    ...
    db.commit()
    db.refresh(cart_item_db)
    return {'message': 'CartItem успешно обновлен'}
...
async def delete_cart_item(cart_item_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {'message': 'CartItem удален'}