...
store_menu_router = APIRouter(prefix='/store-menu', tags=['StoreMenu'])
...
async def detail_store_menu(store_menu_id: int, db: Session = Depends(get_db)):
    ...
    if not store_menu_db:
        raise HTTPException(detail='StoreMenu не найден', status_code=400)
    return store_menu_db
...
async def update_store_menu(
    store_menu_id: int,
    store_menu: StoreMenuInputSchema,
    ...
    for key, value in store_menu.dict().items():
        setattr(store_menu_db, key, value)

    db.commit()
    db.refresh(store_menu_db)
    return {'message': 'StoreMenu успешно обновлен'}
...
async def delete_store_menu(store_menu_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {'message': 'StoreMenu удален'}